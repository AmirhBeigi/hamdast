...
export default async function handler(
  ...
  if (!cell) {
    return res.status(400).json({
      message: "It is mandatory to use the cell parameter.",
    });
  }
  ...
  try {
    ...
    if (req.method === "POST") {
      ...
      const data = await axios.get(
        "https://apigw.paziresh24.com/v1/user/list",
        {
          params: {
            cell: cell?.toString().startsWith("0")
              ? (+cell).toString()
              : cell?.toString(),
            id: -0,
          },
        ...
        subscribers = await notificationPB
          ...
          .getFullList({
            filter: `created <= "${moment()
              .subtract(10, "hour")
              .utc()
              .format("YYYY-MM-DD HH:mm:ss")}" && paziresh24_user_id="${
              user?.id
            }"`,
            headers: {
