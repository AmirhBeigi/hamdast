...
export default async function handler(
  ...
  if (req.method === "GET") {
    const prodcuts = await pb.collection("products").getOne(id as string);
  ...
  if (req.method === "PUT") {
    ...
    const prodcuts = await pb.collection("products").update(id as string, {
      title,
      description,
      price,
      app: app_id,
  ...
  if (req.method === "DELETE") {
    ...
    const prodcuts = await pb.collection("products").delete(id as string);
